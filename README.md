...
Проект можно развивать и улучшать дальше. В рамках текущего задания принципиально не настраивалась полноценная безопасность на уровне приложения: нет аутентификации, авторизации, ролей, JWT/OAuth2, сервисных токенов и проверки прав доступа. Это осознанное упрощение учебной реализации, а не рекомендуемая конфигурация для production.

Доступ к внутренним маршрутам ограничен на сетевом уровне:

- наружу публикуется только nginx gateway на `127.0.0.1:8080`;
- порты микросервисов `8081` и `8082` на хост не публикуются;
- gateway пропускает публичные `/api/**`, но возвращает `404` для `/internal/**`;
- микросервисы обращаются к `/internal/**` только внутри закрытой Docker-сети по DNS-именам `equipment-service` и `maintenance-service`.

Такая изоляция подходит для текущего локального проекта, но не заменяет application-level security. В production внутренние вызовы дополнительно защищают, например, mTLS, service-to-service авторизацией, сетевыми политиками и сервисными credentials.

Soft delete в проекте не реализован намеренно. Его семантика зависит от бизнес-требований: можно добавить отдельный признак удаления (`deleted`, `deletedAt`), связать логическое удаление с существующими статусами, ввести дополнительные статусы, вести архив или использовать другой механизм. Также нужно заранее определить, должны ли удалённые записи участвовать в поиске, проверках уникальности, связях и отчётности. Поэтому это прежде всего вопрос бизнес-логики, а не только техническое добавление одного поля.
